...
# Initialize video capture from the existing video file
input_video = 'example-use-out9.mp4'
...
output_video = 'example-use-out10.mp4'
fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # Codec for MP4
video_writer = cv2.VideoWriter(output_video, fourcc, fps, (frame_width, frame_height))
...
while video_capture.isOpened():
    ...
    # Draw the initial circle on the first frame
    if frame_idx == 0:
        cv2.circle(frame, coords1, 10, (0, 0, 255), -1)
        coords = coords1
    ...
    # Draw the trajectory and bounding box on the current frame
    for point in traj:
        cv2.circle(frame, (int(point[0]), int(point[1])),  2, (0, 255, 255), -1)
    for i in range(len(traj) - 1):
        cv2.line(frame, (int(traj[i][0]), int(traj[i][1])), (int(traj[i+1][0]), int(traj[i+1][1])), (0, 255, 255), 1)
